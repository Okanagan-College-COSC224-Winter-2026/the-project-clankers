...
# ---------------------------------------------------------------------------
# Late-submission penalty exclusion
# ---------------------------------------------------------------------------

def test_no_penalty_for_not_reviewing_late_submissions(test_client):
    """A student should not be penalized for incomplete evaluations when
    the only unreviewed peers submitted after the due date."""
    teacher = _make_user("Teacher Late", "teacher-late@gb.test", role="teacher")
    student_a = _make_user("Student A", "student-a-late@gb.test", student_id="LATE01")
    student_b = _make_user("Student B", "student-b-late@gb.test", student_id="LATE02")
    student_c = _make_user("Student C", "student-c-late@gb.test", student_id="LATE03")
    db.session.flush()

    course = _make_course(teacher.id, "Late Penalty Course")
    db.session.flush()

    db.session.add_all([
        User_Course(userID=student_a.id, courseID=course.id),
        User_Course(userID=student_b.id, courseID=course.id),
        User_Course(userID=student_c.id, courseID=course.id),
    ])
    db.session.flush()

    due = datetime.now(timezone.utc) - timedelta(days=1)
    assignment = _make_assignment(
        course.id, "Late Test Assign", external_review=True, due_date=due,
    )
    db.session.flush()

    criteria = _make_rubric_criteria(assignment.id, score_max=100)

    # A and B submit on time (before the due date)
    on_time = due - timedelta(hours=2)
    sub_a = StudentSubmission(
        assignment_id=assignment.id, student_id=student_a.id,
        filename="a.pdf", file_path="a.pdf",
    )
    sub_b = StudentSubmission(
        assignment_id=assignment.id, student_id=student_b.id,
        filename="b.pdf", file_path="b.pdf",
    )
    db.session.add_all([sub_a, sub_b])
    db.session.flush()
    sub_a.submitted_at = on_time
    sub_b.submitted_at = on_time

    # C submits late (after the due date)
    sub_c = StudentSubmission(
        assignment_id=assignment.id, student_id=student_c.id,
        filename="c.pdf", file_path="c.pdf",
    )
    db.session.add(sub_c)
    db.session.flush()
    sub_c.submitted_at = due + timedelta(hours=5)
    db.session.flush()

    # Student A reviews Student B (on-time) but NOT Student C (late)
    _add_review(assignment.id, student_a.id, student_b.id, criteria.id, 85)

    # Set an incomplete evaluation penalty so we can detect if it's applied
    policy = CourseGradePolicy.get_or_create(course.id)
    policy.incomplete_evaluation_penalty_percent = 20.0
    db.session.commit()

    # Teacher checks the gradebook — Student A should NOT be penalized
    _login(test_client, "teacher-late@gb.test", "password")
    resp = test_client.get(f"/class/{course.id}/gradebook/student/{student_a.id}")
    assert resp.status_code == 200

    payload = resp.get_json()
    entry = payload["assignments"][0]
    # Student A's peer evaluation should be "complete" since the only
    # unreviewed student (C) submitted late
    assert entry["peer_evaluation"]["status"] == "complete"
    # No penalty should be applied
    assert entry["penalty_applied_percent"] == 0.0
